 ...
 */
void getRtc(char * rtc_str) {
	//UC rxd_data[7];
	char time_str[20]={0,};
	//printf("I2C DS3231 RTC\n\r");
	i2c_configure(I2C_1, SYS_FREQ, I2C_FREQ); //System clock =75MHz and I2C clock =100 kHz
	//i2c_initialize(0);
	...
	printf("\n\r set value %x-%x-%x, %x:%x:%x,", gRTCtime.mday, gRTCtime.mon,
				gRTCtime.year_s, gRTCtime.hour, gRTCtime.min, gRTCtime.sec);

	i2c_set_rtc_DS3231(0, DS3231_TIME_CAL_ADDR, DS3231_I2C_ADDR_WR);
	printf("write completed rtc\n\r");
	
	while (1) {
		i2c_get_rtc_DS3231(I2C_1, DS3231_I2C_ADDR_WR,
		DS3231_I2C_ADDR_RD, DS3231_TIME_CAL_ADDR);

		sprintf(rtc_str,"Date : %02x/%02x/%02x,", gRTCtime.mday, gRTCtime.mon,
				gRTCtime.year_s);
		...
		sprintf(time_str,"\n\nTime : %02x:%02x:%02x", gRTCtime.hour, gRTCtime.min, gRTCtime.sec);
		strcat(rtc_str,time_str);
	udelay(20000);
	//}
...
clock_t total_time=0;
	UL distance=0;
		...
		printf("\n\r INFO: The time & date will be displayed on OLED Display");
		printf("\n\r *****************************************************************************\n\r\n\r");
